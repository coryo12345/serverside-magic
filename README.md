...
3. Run `./gradlew jar` to pack all resources into a jar located at `build/libs/*.jar`
...
2. The `server.properties` file needs to be updated, to set a server resourcepack. Set it to: `<server-address>:8080/assets/servermagic-resourcepack.zip`
...
`/setmagicurl <server-address:8080>`
